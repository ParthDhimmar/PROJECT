...
class MainWidget(RelativeLayout):
    ...
    def __init__(self, **kwargs):
        ...
        if self.is_desktop():
            ...
            # self._keyboard.bind(on_key_down=self.K_LEFT)
            self._keyboard.bind(on_key_up=self.on_keyboard_up)
            # self._keyboard.bind(on_key_up=self.K_RIGHT) 
    ...
    def is_desktop(self):
        if platform in ('linux', 'windows', 'macosx','android'):
            return True
        return False
    ...
    def init_ship(self):
        with self.canvas:
            # Color(.2, 1, .6)
            if Color(self.r, self.g, self.b)==0:
                self.r=1
                self.g=1
                self.b=1
            else:    
