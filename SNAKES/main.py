...
class Cell(Widget):
    ...
    def graphical_pos_attach(self, smooth_motion=None):
        to_x, to_y = self.actual_pos[0] - self.graphical_size[0] / \
            2, self.actual_pos[1] - self.graphical_size[1] / 2
    ...
    def step_by(self, direction, **kwargs):
        self.move_by(self.actual_size[0] * direction[0],
                     self.actual_size[1] * direction[1], **kwargs)
...
class Worm(Widget):
    ...
    def lengthen(self, pos=None, direction=(0, 1)):
        if pos is None:
            ...
            pos = (px, py)
        self.cells.append(
            Cell(*pos, self.cell_size, margin=self.config.MARGIN))
...
class Form(Widget):
    ...
    def align_labels(self):
        try:
            self.popup_label.pos = (
                (self.size[0] - self.popup_label.width) / 2, self.size[1] / 2)
            self.score_label.pos = (
                (self.size[0] - self.score_label.width) / 2, self.size[1] - 80)
        except:
            print(self.__dict__)
            assert False
    ...
    def update(self, _):
        if not self.game_on:
            return
        self.worm.move(self.cur_dir, smooth_motion=(
            self.smooth, self.config.INTERVAL))
